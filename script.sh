...
function install_ss_panel_mod_UIm(){
	...
	#克隆项目
	git clone https://github.com/NaclFire/ss-panel-v3-mod_Uim-resource.git tmp && mv tmp/.git . && rm -rf tmp && git reset --hard
	#复制配置文件
...
EOF
	...
	wget -N -P  /home/wwwroot/default/vendor/composer --no-check-certificate "https://raw.githubusercontent.com/NaclFire/ss-panel-v3-mod_Uim/master/autoload_classmap.php"
...
function Libtest(){
	...
	if [ "$libAddr" == "$GIT" ];then
		libAddr='https://raw.githubusercontent.com/NaclFire/ss-panel-v3-mod_Uim/master/libsodium-1.0.17.tar.gz'
	else
...
function install_node(){
	...
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/NaclFire/ss-panel-v3-mod_Uim/master/supervisord.conf	
	supervisord
	#iptables
	iptables -F
	iptables -X  
...
function NEW_NODE(){
     wget -N --no-check-certificate  https://raw.githubusercontent.com/NaclFire/ss-panel-v3-mod_Uim/master/node.sh && bash node.sh
}
